...
![購票流程圖](docs/flow-diagram.png)

購票流程如下：
1. 國際粉絲選擇偏好語言進入系統
2. 瀏覽場次，選擇票種（早鳥票、一般票、VIP 票）並加入購物車
3. 選擇支付方式（Line Pay、Apple Pay、Visa、信用卡、街口支付）
4. 送出訂單，系統產生 QR Code 電子票券並發送確認信
5. 訂單儲存至歷史紀錄，系統同步更新剩餘票數
6. 主辦方即時收到銷售數據與收益統計
...
| 測試項目 | 說明 |
|---------|------|
| 用戶登錄測試 | 以身分證字號查詢使用者資料，驗證語言設定正確 |
| 購票流程測試 | 新增訂單與支付記錄，驗證資料正確寫入 |
| 座位選擇邊界測試 | 驗證選票數量介於 1～6 張之間，超過上限則拒絕 |
...
├── docs/
│   ├── flow-diagram.png    # 購票流程圖
│   ├── er-diagram.png      # E-R 關聯圖
